...
void ackPopupInfo(const char *info)
{
  if (info == echomagic)
  {
    statusScreen_setMsg((uint8_t *)info, (uint8_t *)dmaL2Cache + ack_index);
  }  
  if (infoMenu.menu[infoMenu.cur] == menuTerminal) return;
  if (infoMenu.menu[infoMenu.cur] == menuStatus && info == echomagic) return;
  
  popupReminder((uint8_t* )info, (uint8_t *)dmaL2Cache + ack_index);
}
...
{ 
  ...
  else
  {
    ...
    {
      popupReminder((uint8_t* )"Repeatability Test", (uint8_t *)dmaL2Cache + ack_index-5);
      //popupReminder((uint8_t* )"Standard Deviation", (uint8_t *)&infoCmd.queue[infoCmd.index_r].gcode[5]);
    }
    ...
    else if(ack_seen(bsdprintingmagic))
    {
      ...
      char *ptr;
      uint32_t position = strtol(strstr(dmaL2Cache, "byte ")+5, &ptr, 10); 
...
//      powerFailedCache(position);
    ...
    {
      for(uint8_t i = 0; i < aCount(ignoreEcho); i++)
      {
